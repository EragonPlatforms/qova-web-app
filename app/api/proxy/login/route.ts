...
export async function POST(req: Request) {
  try {
    ...
    // console.log("Received request body for login:", body);
    ...
    // console.log("Raw API Response from /user:", text);

    let data;
    try {
      data = JSON.parse(text);
    } catch (jsonError) {
      console.error("Invalid JSON from backend (/user):", text);
      return NextResponse.json(
    ...
    console.error("Proxy API error (login):", error);
    return NextResponse.json(
